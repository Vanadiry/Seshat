...
通常情况下，你只需要备份主目录下的 `tracker` 文件夹即可。如果你使用了 ELO 评分，则还应备份主目录下的 `user/elo` 文件夹。

最小化的情况，只需要备份主目录下的所有 `.toml` 文件，以及主目录下的 `user/elo/history.json` 文件。  
依靠这些文件，Seshat 能重建你的内容。

如果你需要完全的数据可控，则需要备份整个主目录。  
