...
本指南以 Server 版为例，Desktop 版除启动方式外其余一致。

## 网络

如果你的网络无法直接访问“番组计划”，可以为后端配置代理。  
详见 [docs/config.md](config.md) 中的 proxy 部分。

如果你的代理工具支持 TUN 模式，启动它，将无须单独为 Seshat 配置代理。  
（视工具不同，此模式可能叫作：虚拟网卡模式、增强模式、接管所有流量、透明代理等）

## 拉取动画

程序启动后，访问 `http://127.0.0.1:12500/` 打开 WebUI（Desktop 版本直接打开应用即可）。  
...
对于匿名访问无法获取的受限条目，需要配置个人访问令牌。

在 [番组计划个人令牌](https://next.bgm.tv/demo/access-token) 页面中，可以创建令牌。  
如果不是频繁拉取受限条目，建议将有效期设为最短的 7 天，以防止令牌泄露。

生成令牌后，填入 `config.toml` 的 `[access]` 段：

```toml
[access]
bangumi_access_token = "令牌"
```

修改后需重启 Seshat 生效。令牌会用于后端拉取数据，以及在回退/自定义后端模式下前端请求远程 API 时自动携带。
...
如果你不希望某些条目出现在评分配对中（例如已删除或不想评价的条目），可以编辑主目录下的 `user/elo/exclude.toml`：

```toml
ids = [184740, 123456]
```

支持多行逗号分隔，格式与 Tracker 一致。修改后刷新评分页即可生效。

### 删除评分记录

删除主目录的 `user/elo/history.json` 中，对应的记录，然后重建 ELO 即可。
...
左侧是你能够直接修改的前端配置，右侧是后端配置，如需修改，需要编辑 `config.toml` 并重启。

标题展示方式，可以控制标题展示的内容。大多数页面都支持这个修改。  
默认为原文优先，即大标题原文，副标题中文。设置为中文优先可以将其交替位置。

条目名称高亮，可以控制详情页的简介和数据栏中，是否高亮展示本地已有的条目。
如果你有非常多数据：点进一个详情页，需要加载 1 秒左右的话，可以考虑关掉它。这能显著降低加载时间。

如果你有“番组计划”的账号，在“Bangumi 用户 ID”中输入它。  
保存后，在管理数据按钮中，刷新有用户数据。Seshat 将可以展示你的收藏状态。
...
对比所有 Tracker 中记录的 ID 与本地已缓存的条目，仅拉取本地不存在的新条目。  
适合往 Tracker 里加了新 ID 之后使用。

### 补充数据

遍历本地所有已缓存的条目，重新请求 API，检查并拉取新增的人物、角色、剧集等关联数据。  
适合条目本身已有，但关联数据不完整的情况。

### 刷新全部

根据 Tracker 拉取全部条目数据，直接覆盖本地旧数据。  
本地存在但本次未获取到的数据不会被删除。

### 刷新元数据

重新拉取所有条目的基本信息（info.json），不下载图片。  
适合只希望更新标题、简介等属性，不在意图片的情况。
...
Seshat 完全依赖 Tracker 中记录的 ID 来拉取数据。  
通常情况下，你只需要备份主目录下的 `tracker` 文件夹即可。如果你使用了 ELO 评分，则还应备份主目录下的 `user/elo/history.json`。
依靠这些文件，Seshat 能重建你的内容。

如果你需要完全的数据可控，则建议备份整个主目录。  
这能够在完全离线的情况下复现已储存的记录。
